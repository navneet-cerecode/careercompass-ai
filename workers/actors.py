...
def build_job_discovery_actor(
    *,
    broker: Broker,
    runner: BackgroundTaskRunner,
    operation: RunJobDiscovery,
    app_settings: Settings,
    actor_name: str = "job_discovery",
) -> dramatiq.Actor:
    """Build the product job-discovery actor."""
    ...
    def job_discovery(task_id: str, user_id: str | None = None) -> None:
        try:
            parsed_task_id = UUID(task_id)
            parsed_user_id = UUID(user_id) if user_id is not None else None
        except ValueError:
            logger.warning("Rejected discovery message with invalid identifiers.")
            return
        ...
        try:
            ...
            logger.warning("Rejected discovery message for an unavailable task.")
            return
        except Exception as error:
            logger.error(
                "Discovery runtime failed before task completion; error_type=%s",
                type(error).__name__,
            )
            raise RetryableTaskExecution("task_runtime_unavailable") from None
