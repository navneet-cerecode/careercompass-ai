...
import { GET } from "@/app/api/cover-letters/[coverLetterId]/export/route";
...
describe("cover letter export route", () => {
  it("streams an authenticated document without exposing the access token", async () => {
    const bytes = new TextEncoder().encode("%PDF-private-cover-letter");
    ...
    const response = await GET(
      new Request(`http://localhost/api/cover-letters/${id}/export?format=pdf`),
      { params: Promise.resolve({ coverLetterId: id }) },
    );
  ...
  it("rejects unsupported formats before calling the backend", async () => {
    ...
    const response = await GET(
      new Request(`http://localhost/api/cover-letters/${id}/export?format=html`),
      { params: Promise.resolve({ coverLetterId: id }) },
    );
